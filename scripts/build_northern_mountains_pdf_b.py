#!/usr/bin/env python3
"""Build the knee-aware Version B northern AT mountain guide PDF."""
...
import build_northern_mountains_pdf as base


ROOT = Path(__file__).resolve().parents[1]
DATA_PATH = ROOT / "apps/openclaw-web/src/lib/data/northern-mountains-guide-b.json"
OUTPUT_PATH = ROOT / "output/pdf/hogg-country-at-mountains-mile-1850-to-katahdin-version-b.pdf"
PUBLIC_PATH = ROOT / "apps/openclaw-web/static/guides/hogg-country-at-mountains-mile-1850-to-katahdin-version-b.pdf"
...
def draw_page(canvas: Any, document: BaseDocTemplate) -> None:
    ...
    canvas.drawString(MARGIN_X, PAGE_HEIGHT - 14, "MOUNTAINS AHEAD | VERSION B CANDIDATE")
...
def cover_story(data: dict[str, Any]) -> list[Any]:
    ...
    title_content = Table([[
        Table([
            [paragraph("VERSION B CANDIDATE | DAD'S 2026 NOBO FIELD REFERENCE", STYLE["cover_flag"])],
            [Spacer(1, 12)],
    ...
    story.append(paragraph(
        "<b>READ VERSION B:</b> UP is the current movement score. DOWN is the knee-load screen. "
...
def method_and_sources(data: dict[str, Any]) -> list[Any]:
    story: list[Any] = [
        PageBreak(),
        paragraph("VERSION B METHOD", STYLE["section_kicker"]),
    ...
    story.extend([
        ...
        Spacer(1, 16),
        paragraph(
            f"Candidate compiled {data['generatedAt']}. Every displayed trail mile is re-derived from "
...
def build_pdf(data: dict[str, Any], destination: Path) -> None:
    ...
    frame = Frame(
        ...
        topPadding=0,
        bottomPadding=0,
        id="candidate-guide-frame",
    )
    document.addPageTemplates([PageTemplate(id="candidate-guide", frames=[frame], onPage=draw_page)])
