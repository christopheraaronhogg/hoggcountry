...
Generated at: 2026-06-29T06:16:37.733Z
...
- Target iOS build for Dad Eval Lab: `1.0 (32)`.
- Target build meets suite requirement: yes.
- Recorded Dad Pilot build: `1.0 (31)`.
- Recorded Dad Pilot build meets suite requirement: yes.
- Latest Dad Pilot proof: `docs/launch/proof/ios-testflight-build-31-ledger-2026-06-29.md` (1.0 (31), IN_BETA_TESTING, checked 2026-06-29T05:59:27.383Z).
- Latest Dad Pilot gates: 5/5 checked; targetReadyForDad yes.
- Latest local target prep: `docs/launch/proof/ios-testflight-build-32-prep-2026-06-29.md` (1.0 (32), checked 2026-06-29T06:16:15Z; not App Store Connect proof).
- Newer Xcode target pending App Store Connect: yes.
- Snapshot checkout SHA: `c97b8dc52931b45ea1d49c9991b996ab43e7223d`.
- Latest native upload source: `docs/launch/proof/ios-testflight-attempt-2026-06-29T05-54-01-956Z.md` (repo SHA `e0a3a794a8b44b12cc6f5f0e59db21fb7681823c` from `.scout-artifacts/ios-testflight/2026-06-29T05-54-01-956Z/01-repo-sha.log`).
- Latest native upload attempt: `docs/launch/proof/ios-testflight-attempt-2026-06-29T05-54-01-956Z.md` (passed, upload requested yes, checked 2026-06-29T05:55:11.883Z).
- Snapshot checkout newer than latest native upload: yes.
- Snapshot native app source newer than latest native upload: no.
- Imported full device runs: 11.
...
- Use now: Dad can run the suite on the currently approved Dad Pilot build `1.0 (31)`; do not wait for target `1.0 (32)` unless latest-code proof is required.
- Latest-code target: `1.0 (32)` is the Xcode target/local candidate, but it still needs App Store Connect upload/refresh proof before it is the latest Dad Pilot build.
...
- Do not count as final proof until: the exported run shows TestFlight/iPhone context and app build satisfying `1.0 (>= 13)`; build `1.0 (32)` only counts as latest-code proof after Dad Pilot refresh shows targetReadyForDad.
...
- Simulator preflight evidence: Latest simulator/debug local preflight device-local-ai-20260629T053506Z is clean: 100/100 cases, complete tools/source evidence, app 1.0 (31), install=debug, model=gemma-4-E2B-it-litert-lm.
- Simulator full runs: 11; partial runs: 0.
- Boundary: simulator/debug local preflight drives iteration but does not replace final TestFlight/iPhone proof.
- Latest simulator Run 100: `device-local-ai-20260629T053506Z` (100/100 cases, tools complete 100/100, sources complete 100/100, answer scan clean with 0 flagged).
- Final-proof mismatch by design: device-local-ai-20260629T053506Z (install=debug, expected testflight).
...
- [x] Full-suite tool routing proof: 11 current full run(s) with all required tools hit and source evidence recorded
- [x] Simulator/debug local full-suite preflight: Latest simulator/debug local preflight device-local-ai-20260629T053506Z is clean: 100/100 cases, complete tools/source evidence, app 1.0 (31), install=debug, model=gemma-4-E2B-it-litert-lm
- [x] Dad Pilot has current suite-required TestFlight build: Dad Pilot has a suite-compatible TestFlight build; newer Xcode target is pending App Store Connect: target 1.0 (32); suite requires 1.0 (>= 13); Dad Pilot records 1.0 (31); latest native upload suite 2026-06-28.5 (fnv1a32:741b2381); current suite 2026-06-28.5 (fnv1a32:741b2381)
- [ ] Full TestFlight/iPhone Eval Lab run imported: No current full suite-compatible TestFlight/iPhone run found; 11 full device-on-device-gemma run(s) failed final-proof context: device-local-ai-20260628T235310Z (install=debug, expected testflight); device-local-ai-20260629T001151Z (install=debug, expected testflight); device-local-ai-20260629T005630Z (install=debug, expected testflight)
...
- Xcode Release target: `1.0 (32)` from `mobile/ios/App/App.xcodeproj/project.pbxproj`.
- Signing team/profile: `3CFU9J87A5` / `Hoggcountry App Store Connect`.
- Latest successful native upload proof: `docs/launch/proof/ios-testflight-attempt-2026-06-29T05-54-01-956Z.md` (passed, checked 2026-06-29T05:55:11.883Z).
- Latest native upload attempt: `docs/launch/proof/ios-testflight-attempt-2026-06-29T05-54-01-956Z.md` (passed, checked 2026-06-29T05:55:11.883Z).
- Latest successful native upload repo SHA: `e0a3a794a8b44b12cc6f5f0e59db21fb7681823c` from `.scout-artifacts/ios-testflight/2026-06-29T05-54-01-956Z/01-repo-sha.log`.
...
1. Confirm the phone build path above. For the next suite run, Dad can run the suite on the currently approved Dad Pilot build `1.0 (31)`; do not wait for target `1.0 (32)` unless latest-code proof is required.
...
- Native context: TestFlight iPhone install, app build satisfying `1.0 (>= 13)`; current Dad Pilot proof records `1.0 (31)`, while latest Xcode target is `1.0 (32)`.
...
Run 100 now on the suite-compatible Dad Pilot TestFlight build 1.0 (31); newer target 1.0 (32) is pending upload. Repo changes after the latest successful native upload are outside native app source, so no fresh TestFlight upload is needed before Dad's Run 100. Open Settings > Scout Eval Lab, run Run 100, and Share the JSON. While waiting for the file, leave npm run wait:scout-local-ai-device-run running; status also checks /Users/chrishogg/Downloads and will use npm run prepare-review:scout-local-ai-device-run -- --run latest if the export lands there. If Dad sends copied JSON text instead of a file, use npm run receive:scout-local-ai-device-run -- --clipboard or paste into npm run receive:scout-local-ai-device-run -- --stdin; the receiver saves it to the inbox, inspects it, and prepares the same review path as npm run prepare-review:scout-local-ai-device-run -- --run inbox when it is final-ready.
