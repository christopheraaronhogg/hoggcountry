...
class SyncEngine {
	...
	async #ensureRestoredThenDrain(): Promise<void> {
		...
		try {
			...
			if (!browser || !navigator.onLine || !this.#restore) {
	...
	async #runRestore(epoch: number): Promise<boolean> {
		if (!browser || !this.#restore || !cloudAuth.signedIn || !navigator.onLine) return false;
		...
		if (epoch !== this.#authEpoch || !cloudAuth.signedIn) return false;
	...
	async #drain(): Promise<void> {
		if (!browser || this.#draining || this.#restoring || !this.#hydrated) return;
		if (!cloudAuth.signedIn) {
			this.status = 'signed-out';
			return;
		}
		...
		try {
			...
			if ((result.applied?.length ?? 0) > 0) this.lastBackupAt = nowIso();
			this.#persistNow();
