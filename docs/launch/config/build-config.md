...
## What this means
- **iOS** project-file blockers are cleared for the privacy manifest, shared scheme, LiteRT-LM package wiring, Release signing, and TestFlight archive/upload lane. Build `27` is live in Dad Pilot through the public TestFlight link, and App Store Connect reports it `VALID` with external state `IN_BETA_TESTING`, but that upload contains eval suite `2026-06-27.2`. The current local target is build `28`; a read-only App Store Connect refresh on 2026-06-28 confirmed build `28` is not uploaded yet, so it must be uploaded and attached before Dad runs the current suite `2026-06-28.4`. Scout AI still needs the full 100-question Eval Lab export on a real TestFlight iPhone before Dad-ready claims.
