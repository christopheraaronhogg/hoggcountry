...
- Build `1.0 (22)` is live in Dad Pilot, Apple beta review is approved, and App
  Store Connect reports external state `IN_BETA_TESTING`.
- Latest native upload proof for build `22`:
  `docs/launch/proof/ios-testflight-attempt-2026-06-27T23-56-37-024Z.md`.
- Dad Pilot/App Store Connect proof for build `22`:
  `docs/launch/proof/ios-testflight-build-22-2026-06-27.md`.
- Build `21` was removed from Dad Pilot only after build `22` was attached and
  externally available.
- The local Xcode target is build `22`, matching the current Dad Pilot build.
- Dad should now run `Run 100` on build `22`; the suite allows `1.0 (>= 13)`,
  but build `22` is the latest-source TestFlight proof.
...
- Account/TestFlight proof: build `1.0 (22)` is attached to Dad Pilot and
  externally available.
- Native upload proof: build `22` was archived, exported, and uploaded from the
  native app source in commit `ed706558fe1ee05c6e6dd3de54d9396f052b39a0`.
- Previous Dad Pilot upload: build `21` was archived and uploaded from commit
  `f541be4501a6d42ea9a3e4146e5279cf60c0ae59`, then replaced by build `22`.
- Latest-source TestFlight proof: local Xcode target `1.0 (22)` matches Dad
  Pilot build `1.0 (22)`.
