...
Snapshot of the native build config as of 2026-06-27. Account-bound store
...
| CURRENT_PROJECT_VERSION (local build target) | `21` | live in Dad Pilot; latest-source upload/refresh proof recorded |
...
## What this means
- **iOS** project-file blockers are cleared for the privacy manifest, shared scheme, LiteRT-LM package wiring, Release signing, and TestFlight archive/upload lane. Build `21` is live in Dad Pilot through the public TestFlight link, and App Store Connect reports it `VALID` with external state `IN_BETA_TESTING`. Scout AI still needs model-download/runtime smoke proof and the full 100-question Eval Lab export on a real TestFlight iPhone before Dad-ready claims.
