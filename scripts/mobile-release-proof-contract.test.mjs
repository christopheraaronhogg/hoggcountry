...
test('mobile release proof next mode prints unresolved gates and evidence stubs', () => {
	...
	assert.doesNotMatch(result.stdout, /dad-testflight-invite/u);
...
test('TestFlight helper is a repeatable archive/export/upload lane', () => {
	...
	assert.match(helper, /function redactCommandArgs/u);
	assert.match(helper, /-authenticationKeyIssuerID/u);
	assert.match(helper, /'<redacted>'/u);
