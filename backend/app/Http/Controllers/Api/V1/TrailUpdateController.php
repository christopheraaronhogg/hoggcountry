...
class TrailUpdateController extends ApiController
{
    ...
    public function storeMediaChunk(Request $request, string $uploadId, int $chunkIndex): JsonResponse
    {
        ...
        $this->scratchDisk()->put($this->chunkPath($uploadId, $chunkIndex), $body);
    ...
    public function completeMediaUpload(Request $request, string $uploadId): JsonResponse
    {
        ...
        for ($index = 0; $index < $chunkCount; $index++) {
            if (! $this->scratchDisk()->exists($this->chunkPath($uploadId, $index))) {
                $missing[] = $index;
            }
        }
        ...
        $absolutePath = $this->scratchFilePath(self::UPLOADS_DIR.'/'.$this->sanitizeId($uploadId).'/assembled/'.$mediaKey);
        ...
        try {
            for ($index = 0; $index < $chunkCount; $index++) {
                $chunk = $this->scratchDisk()->get($this->chunkPath($uploadId, $index));
                fwrite($out, $chunk);
            }
        } finally {
            fclose($out);
        ...
        if (! $this->putFileOnStorageDisk($mediaPath, $absolutePath)) {
            $this->deleteLocalFile($absolutePath);
        ...
        try {
            $completed = [
                ...
                'mediaDerivatives' => $this->makeImageDerivatives($absolutePath, (string) ($upload['contentType'] ?? ''), $mediaKey),
        ...
        $this->scratchDisk()->put($this->completedPath($uploadId), json_encode($completed, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
        $this->deleteUpload($uploadId);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! $this->writeUpdates($updates)) {
            $this->deleteUpdateMedia($update);

            return $this->withCors($this->fail('trail_updates_write_failed', 'Could not save the trail update.', 500), $request);
        }
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        ...
        if (! $this->writeUpdates($remaining)) {
            return $this->withCors($this->fail('trail_updates_write_failed', 'Could not delete the trail update.', 500), $request);
        }

        if (is_array($target)) {
            $this->deleteUpdateMedia($target);
        }

        return $this->withCors($this->ok(['ok' => true]), $request);
    }

    public function media(Request $request, string $id): JsonResponse|StreamedResponse
    {
    ...
    public function mediaVariant(Request $request, string $id, string $variant): JsonResponse|StreamedResponse
    {
    ...
    private function servePublicMedia(Request $request, string $path, string $contentType): JsonResponse|StreamedResponse
    {
        $disk = $this->trailUpdateDisk();
        if (! $disk->exists($path)) {
            return $this->withCors($this->fail('trail_updates_media_missing', 'Media not found.', 404), $request);
        }
        ...
        $size = $this->storageFileSize($path);
        if ($size !== null) {
    ...
    {
        $raw = $this->trailUpdateDisk()->exists(self::UPDATES_KEY)
            ? $this->trailUpdateDisk()->get(self::UPDATES_KEY)
            : '';
     ...
     */
    private function writeUpdates(array $updates): bool
    {
        return (bool) $this->trailUpdateDisk()->put(self::UPDATES_KEY, json_encode([
    ...
    private function storeDirectMedia(?UploadedFile $file, string $id): ?array
    {
        ...
        $mediaPath = $file->storeAs(self::MEDIA_DIR, $mediaKey, $this->trailUpdateDiskName());
        ...
        return [
            ...
            'mediaDerivatives' => $this->makeImageDerivatives($file->getPathname(), $contentType, $mediaKey),
        ];
    }
     ...
     * @param  array<string,mixed>  $update
     */
    private function deleteUpdateMedia(array $update): void
    {
        if (! empty($update['mediaPath'])) {
            $this->trailUpdateDisk()->delete((string) $update['mediaPath']);
        }

        foreach ($this->mediaDerivativePaths($update) as $path) {
            $this->trailUpdateDisk()->delete($path);
        }
    }
     ...
     * @return array<string,array<string,mixed>>
     */
    private function makeImageDerivatives(string $sourcePath, string $contentType, string $mediaKey): array
    {
        ...
        foreach (self::IMAGE_DERIVATIVES as $variant => $config) {
            ...
            $absoluteVariantPath = $this->scratchFilePath(self::UPLOADS_DIR.'/derivatives/'.$base.'-'.$variant.'-'.Str::lower(Str::random(8)).'.webp');

            if (imagewebp($target, $absoluteVariantPath, $config['quality'])) {
                $size = filesize($absoluteVariantPath) ?: 0;
                if ($this->putFileOnStorageDisk($variantPath, $absoluteVariantPath)) {
                    $derivatives[$variant] = [
                        'path' => $variantPath,
                        'type' => 'image/webp',
                        'width' => $targetWidth,
            ...
            $this->deleteLocalFile($absoluteVariantPath);
            imagedestroy($target);
        }
    ...
    private function storeVideoCover(string $dataUrl, string $mediaKey): array
    {
        ...
        if (! $this->trailUpdateDisk()->put($coverPath, $binary)) {
            return [];
        }
    ...
    private function consumeCompletedMedia(string $uploadId): ?array
    {
        ...
        if (! $this->scratchDisk()->exists($path)) {
            return null;
        }

        $decoded = json_decode($this->scratchDisk()->get($path), true);
        $this->scratchDisk()->delete($path);
    ...
    {
        $this->scratchDisk()->put($this->uploadPath($uploadId), json_encode($upload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
    }
    ...
    private function deleteUpload(string $uploadId): void
    {
        ...
        $this->scratchDisk()->delete($this->uploadPath($uploadId));
        $this->scratchDisk()->deleteDirectory(self::UPLOADS_DIR.'/'.$uploadId);
    }
    ...
    private function trailUpdateDisk(): FilesystemAdapter
    {
        return Storage::disk($this->trailUpdateDiskName());
    }

    private function trailUpdateDiskName(): string
    {
        $disk = trim((string) config('trail_updates.storage_disk', 'r2'));

        return $disk !== '' ? $disk : 'r2';
    }

    private function scratchDisk(): FilesystemAdapter
    {
        return Storage::disk($this->scratchDiskName());
    }

    private function scratchDiskName(): string
    {
        $disk = trim((string) config('trail_updates.scratch_disk', 'local'));

        return $disk !== '' ? $disk : 'local';
    }

    private function scratchFilePath(string $path): string
    {
        $absolutePath = $this->scratchDisk()->path($path);
    ...
    private function putFileOnStorageDisk(string $path, string $absolutePath): bool
    {
        ...
        try {
            return (bool) $this->trailUpdateDisk()->put($path, $stream);
        } finally {
            fclose($stream);
        }
    }

    private function storageFileSize(string $path): ?int
    {
        try {
            $size = $this->trailUpdateDisk()->size($path);
        } catch (\Throwable) {
            return null;
        }
    ...
    private function deleteLocalFile(string $absolutePath): void
    {
    ...
    {
        return max(1, min(2048, (int) config('trail_updates.max_media_mb', self::DEFAULT_MAX_MEDIA_MB)));
    }
    ...
    private function withCors(JsonResponse|StreamedResponse $response, Request $request): JsonResponse|StreamedResponse
    {
