...
test('status command keeps routing proof separate from missing device proof', async () => {
	...
	assert.equal(status.testflight.targetBuild, '1.0 (23)');
	...
	assert.equal(status.testflight.recordedDadPilotBuild, '1.0 (23)');
	...
	assert.match(gates['testflight-target'].evidence, /Dad Pilot records 1\.0 \(23\)/u);
...
test('status command surfaces target TestFlight build gaps before phone eval', async () => {
	...
	assert.match(status.nextAction.text, /Upload and attach target iOS build 1\.0 \(23\)/u);
...
test('goal audit maps original success criteria without hiding missing device proof', async () => {
	...
	assert.match(requirements['target-testflight-build'].evidence, /Dad Pilot records 1\.0 \(23\)/u);
...
test('Dad handoff command summarizes current TestFlight/iPhone eval next steps', async () => {
	...
	assert.match(result.stdout, /Target iOS build for Dad Eval Lab: `1\.0 \(23\)`/u);
	...
	assert.match(result.stdout, /Latest-code target: `1\.0 \(23\)` is the Xcode target\/local candidate/u);
	...
	assert.match(result.stdout, /Xcode Release target: `1\.0 \(23\)`/u);
	...
	if (uploadRepoShaIsAncestor && nativeAppSourceChangedSinceUpload) {
		...
		assert.match(result.stdout, /Current native app source newer than latest native upload: yes/u);
		assert.match(result.stdout, /Latest-source upload note: upload target build `1\.0 \(23\)`; bump again only if App Store Connect already has build `23`/u);
	...
	assert.match(result.stdout, /npm run refresh:testflight-dad-pilot -- --build 23 --app-version 1\.0/u);
