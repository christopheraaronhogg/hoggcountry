...
Checked at: 2026-06-27T14:02:12Z

## Local candidate

- Base repo SHA before prep: `2628a2aedba7c72696a8018adb72fb705c33fb2e`
...
- Purpose: package the latest Scout chat reading-flow polish and send-scroll
  motion reduction into the next TestFlight candidate, while preserving build
  `19` as the current approved Dad Pilot build until App Store Connect proves
  build `20` is processed and attached.
...
Dad. Build `20` is the latest-source candidate and needs the normal archive,
upload, and Dad Pilot refresh before Dad can install that exact build from
TestFlight.

## Completed locally

- `CURRENT_PROJECT_VERSION` bumped from `19` to `20` for Debug and Release in
  `mobile/ios/App/App.xcodeproj/project.pbxproj`.
- `node --test scripts/scout-local-ai-eval-suite.test.mjs` passed with 103/103
  tests after updating target-build expectations for `1.0 (20)` and preserving
...
- Svelte MCP autofixer reported no issues for `CoachTab.svelte` after the
  transcript search/deep-link control was validated.
...
- `npm test` passed with 337/337 root tests.
