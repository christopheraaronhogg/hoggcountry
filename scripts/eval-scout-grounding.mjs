...
assert.ok(harpers.planOptions.some((option) => option.id === 'harpers-dad-bears-den-longer-finish'), 'Longer Bears Den Dad option should be available');
...
const bearsDenOption = harpers.planOptions.find((option) => option.id === 'harpers-dad-bears-den-longer-finish');
assert.equal(formatAtRouteMileage(bearsDenOption.totalMiles), '21.7', 'Bears Den / VA 7 / Snickers Gap to Harpers Ferry should be the longer south-side option');
const marylandOption = harpers.planOptions.find((option) => option.id === 'harpers-overnight-maryland-ed-garvey-dahlgren');
assert.equal(formatAtRouteMileage(marylandOption.days[0].miles), '6.3', 'Harpers Ferry to Ed Garvey should correct the earlier bad ~19-mile split');
assert.equal(formatAtRouteMileage(marylandOption.days[1].miles), '12.1', 'Ed Garvey to Dahlgren should remain a longer second-day draft split');
...
const badMarylandDraft = `Friday: Harpers Ferry to Ed Garvey Memorial Shelter is about 18.9 miles. Do Ed Garvey and Dahlgren require permits or reservations? Bears Den area road crossings can be tricky.`;
const badMarylandIssues = validateAtRouteAnswerClaims(badMarylandDraft, harpers);
assert.ok(badMarylandIssues.some((issue) => issue.kind === 'bad-mileage'), 'Validator should block the old bad Harpers Ferry to Ed Garvey mileage split');
assert.ok(badMarylandIssues.some((issue) => issue.kind === 'unsafe-camping-rule'), 'Validator should block vague Maryland permit/reservation guessing');
assert.ok(badMarylandIssues.some((issue) => issue.kind === 'unsafe-summit-plan'), 'Validator should warn on hand-wavy Bears Den access language');
const harpersSafeDraft = `This is Harpers Ferry / ATC HQ mental-halfway planning, not Pine Grove. Route validation pass: Keys Gap to Harpers Ferry / ATC HQ is the higher-confidence Dad-friendly finish access; Bears Den / VA 7 / Snickers Gap is a longer south-side option with parking/shuttle confirmation still required. Friday/Saturday draft option: Harpers Ferry to Ed Garvey Memorial Shelter, then Ed Garvey to Dahlgren Backpack Campground. I need to check current Maryland camping rules before recommending an overnight, and I still need current guide mileage, legal overnight, water, parking, shuttle, and weather checks before commitment.`;
assert.deepEqual(validateAtRouteAnswerClaims(harpersSafeDraft, harpers), [], 'Validator should allow Harpers Ferry routing with explicit validation pass, access confidence, and legal/water caveats');
...
assert.ok(clawAgentSource.includes('function renderAtRouteResourceContext'), 'Route-resource context renderer should remain available');
assert.ok(clawAgentSource.includes("skillEnabled(record, 'at-mile-marker-reference')"), 'AT mile-marker skill should gate preloaded route context');
assert.ok(clawAgentSource.includes('Do not return deterministic or hardwired planning answers'), 'Scout must not reintroduce deterministic route replies');
assert.ok(clawAgentSource.includes('Validation-pass rule: before giving day-by-day logistics'), 'Route resource context should require a validation pass before day-by-day logistics');
assert.ok(clawAgentSource.includes('wrong mileage is the dangerous failure mode'), 'System prompt should treat Harpers Ferry / Maryland mileage errors as safety-critical');
assert.ok(clawAgentSource.includes('I need to check current Maryland camping rules before recommending an overnight'), 'System prompt should fail closed on Maryland overnight rules instead of guessing permit/reservation status');
assert.ok(clawAgentSource.includes('Bears Den/VA 7/Snickers Gap is a longer south-side option'), 'System prompt should name practical Bears Den / VA 7 / Snickers Gap access with confidence and verification needs');

console.log('Scout grounding eval passed: Pine Grove, Harpers Ferry, GSMNP, Shenandoah, White Mountains, 100-Mile Wilderness, and Baxter/Katahdin route order, blocked endpoints, bad mileage, non-deterministic route context, validation-pass guidance, permit/camping/water/food/summit guardrails, and shelter-overflow/work-for-stay refusal are active.');
