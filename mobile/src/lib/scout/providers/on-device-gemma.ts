...
const PRETRIP_SCREENSHOT_DRAFT_NOTE =
	'Draft screenshot/save checklist note: 1. Current mile or start location confirmed. 2. Itinerary and check-in plan saved offline. 3. Emergency contacts saved outside Scout. 4. Next resupply, town, or bailout plan saved. 5. Offline map and Scout field-pack/local-model status captured. 6. Key permits, reservations, shuttle, lodging, and medication/allergy summaries saved without private numbers. Review this draft before saving; Scout should not save or overwrite a document unless you explicitly confirm it, and any saved change should be versioned or recoverable.';
...
const SCOUT_TOWN_UPDATE_DRAFT_NOTE =
	'Draft town-exit update note: 1. Current AT mile: confirm before saving. 2. Food and water carry: note what changed. 3. Weather and closures: refreshed or still needs live check. 4. Loadout or gear changes: record anything added, removed, wet, broken, or replaced. 5. Documents: note permits, reservations, shuttle, lodging, or itinerary updates without private numbers. 6. Offline readiness: field pack, maps/docs, Bible, and local AI model checked. 7. Open questions before walking out: water, shelter, town, terrain, and bailout. Review this draft before saving; Scout should not save or overwrite a document unless you explicitly confirm it, and any saved change should be versioned or recoverable.';
const OFFLINE_DOCUMENT_CHECKLIST_DRAFT_NOTE =
	'Draft offline document checklist note: 1. Photo ID saved offline outside Scout. 2. Insurance card saved offline outside Scout. 3. Emergency contacts saved and shared. 4. Medication/allergy summary saved without private numbers. 5. Itinerary and check-in plan saved. 6. Permits, reservations, shuttle, and lodging confirmations saved as summaries. 7. Scout field pack, maps/docs, Bible, and local AI model checked for offline use. Review this draft before saving; Scout should not save or overwrite a document unless you explicitly confirm it, and any saved change should be versioned or recoverable.';
...
const SALVATION_DIRECT_NOTE =
	'## What Scripture says\n\nBelieve on the Lord Jesus Christ. Salvation is received by grace through faith, not earned by works. Repent and turn to God, and call on the name of the Lord.\n\nKey KJV passages:\n\n1. Acts 16:31 — "Believe on the Lord Jesus Christ, and thou shalt be saved."\n2. Acts 3:19 — "Repent ye therefore, and be converted."\n3. Ephesians 2:8-9 — "For by grace are ye saved through faith... Not of works."\n4. Romans 10:13 — "For whosoever shall call upon the name of the Lord shall be saved."\n\nA prayer can express that faith, but the exact words are not magic; the heart turning to Christ is the point.';
...
export function polishOnDeviceAnswer(text: string, prompt: string, toolInvocations: ToolInvocationRecord[] = []): string {
	...
	if (isSalvationPrompt(lowerPrompt)) {
		answer = polishSalvationAnswer(answer);
	}
	...
	if (isDocumentWritingPrompt(lowerPrompt) && !mentionsDocumentWriteConfirmation(answer)) {
		answer = appendSentence(
			answer,
			'Review the draft before saving; Scout should not save or overwrite a document unless you explicitly confirm it, and any saved change should be versioned or recoverable.'
		);
	}
	if (isDocumentWritingPrompt(lowerPrompt) && !mentionsDocumentSaveRecovery(answer)) {
		answer = appendSentence(answer, DOCUMENT_WRITING_SAVE_RECOVERY_NOTE);
	}
...
function polishSalvationAnswer(answer: string): string {
	const cleaned = answer
		.replace(/\bThat is a question about faith, not about the trail\.?/giu, '')
		.replace(/\bDo not add denominational checklists as if they are required by Scout\.?/giu, '')
		.trim();
	return hasSalvationCore(cleaned) ? cleaned : SALVATION_DIRECT_NOTE;
}

function hasSalvationCore(answer: string): boolean {
	const hasChrist = /\b(?:Jesus|Christ|Lord)\b/iu.test(answer);
	const hasFaith = /\b(?:believe|faith|trust)\b/iu.test(answer);
	const hasSaved = /\b(?:saved|salvation|grace|repent|call upon|call on)\b/iu.test(answer);
	return hasChrist && hasFaith && hasSaved;
}
...
function mentionsDocumentSaveRecovery(answer: string): boolean {
...
export function renderSystemContext(request: ProviderRequest): string {
	...
	return [
		...
		toolLines.length ? `Source packet from local search/tools:\n${toolLines.join('\n')}` : '',
		`Use relevant source-packet facts before model memory. If a relevant local tool/source is supplied, lean on it. Cite user-facing sources from these findings, do not invent landmarks, and ask the hiker to confirm volatile facts.`
	]
		.filter(Boolean)
		.join('\n\n');
}

function guidanceForScoutPrompt(prompt: string, toolIds: Set<string>): string[] {
	const hasTool = (id: string) => toolIds.has(id);
	const lines = [
		'Be honest about uncertainty: use "candidate", "verify", or "I don\'t know" when the pack cannot prove something. Never turn candidate water, shelters, towns, weather, or services into guarantees.'
	];

	if (isDirectNextWaterDistancePrompt(prompt) || hasTool('next_water')) {
		lines.push('Water: lead with the nearest actionable water or next reliable source from next_water. Say when it is seasonal/unverified, visually confirm flow, filter or treat collected water, and carry enough to reach verified water if the candidate is dry.');
	}
	if (isDirectNextTownDistancePrompt(prompt) || hasTool('next_town')) {
		lines.push('Town/access: lead with distance to the next loaded town or road/town access. If services are unconfirmed, say so plainly and tell the hiker to confirm shuttle, pickup, lodging, groceries, and hours before relying on it.');
	}
	if (hasTool('weather_lookup') || isWeatherSensitivePrompt(prompt)) {
		lines.push('Weather: distinguish cached pack weather from live proof. For tomorrow, storms, heat, cold, wind, flooding, fire/smoke, or exposed terrain, say what must be refreshed online before relying on it.');
	}
	if (hasTool('upcoming_terrain') || isTerrainDifficultyPrompt(prompt)) {
		lines.push('Terrain/difficulty: use upcoming_terrain as the loaded window. State cached difficulty, gain/loss, grade, steep sections, water, and shelter spacing when present; if missing, say the pack lacks that detail rather than inventing it.');
	}
	if (hasTool('next_shelter') || isShelterOrCampingPrompt(prompt)) {
		lines.push('Shelter/camp: treat shelters/campsites as planning candidates, not guarantees. Weigh daylight, water, current shelter status/crowding, legal rules, weather, fatigue, and a backup option before committing.');
	}
	if (hasTool('loadout_check') || isLoadoutPrompt(prompt)) {
		lines.push('Gear/loadout: start from carried items and conditions. Protect safety basics such as rain, warmth, water treatment, first aid, power/navigation, and sleep system; turn shakedown failures into specific fixes.');
	}
	if (isOfflineSetupPrompt(prompt) || isAirplaneModeCapabilityPrompt(prompt) || isAirplaneModeRehearsalPrompt(prompt) || isModelDownloadingStatusPrompt(prompt)) {
		lines.push('Offline readiness: cover field-pack refresh, current mile, local AI model, offline maps/docs, Bible availability when relevant, battery, and an airplane-mode rehearsal. Scout does not replace inReach, PLB, 911, or the family emergency plan.');
	}
	if (isPersonalDocumentPrompt(prompt) || isDocumentWritingPrompt(prompt)) {
		lines.push('Documents: answer the factual question first, then include a clearly labeled draft only when asked. Keep source-backed facts separate from placeholders/open questions, avoid private numbers in chat, and never imply Scout saved or overwrote a document without explicit confirmation.');
	}
	if (isBiblePrompt(prompt) || hasTool('bible_search')) {
		lines.push('Bible/spiritual: ground the answer in bible_search when available. Quote only KJV verses supplied in the source packet, keep references with quotes, and never fabricate scripture. For salvation, answer directly from scripture about faith, repentance, grace, believing on/calling on the Lord, without adding denominational requirements as required facts. Warm Markdown structure is welcome.');
	}
	if (isPrayerSafePlanPrompt(prompt) || isScaredAloneNightPrompt(prompt)) {
		lines.push('Spiritual support plus safety: do not spiritualize away danger. Pair comfort or prayer with concrete next steps; use loaded shelter/water/town/bailout context as candidates and escalate through emergency channels for real danger, injury, exposure, confusion, or repeated panic.');
	}
	if (isSosSupportCirclePrompt(prompt) || isNoSignalHelpSoonPrompt(prompt) || isOverduePartnerPrompt(prompt) || isMedicalAdviceBoundaryPrompt(prompt) || isStopHikingImmediatePrompt(prompt)) {
		lines.push('Safety/emergency: separate Scout from rescue. Scout cannot call 911 or trigger SOS. Give immediate lower-risk steps and use 911, phone SOS, inReach/PLB, rangers/authorities, or the emergency plan for danger.');
	}

	lines.push('Use the strongest 2-4 source/tool findings visibly. Convert search hits into specific actions instead of generic advice.');
	return lines;
}

function isTerrainDifficultyPrompt(prompt: string): boolean {
	return /\b(?:how hard|hard is today|terrain|climb|descent|elevation|gain|loss|grade|today going to be|today gonna be)\b/u.test(prompt);
}

function isShelterOrCampingPrompt(prompt: string): boolean {
	return /\b(?:shelter|camp|camping|campsite|tent site|sleep|overnight|tonight|after dark|stealth)\b/u.test(prompt);
}

function isLoadoutPrompt(prompt: string): boolean {
