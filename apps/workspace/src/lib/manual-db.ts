...
const DB_NAME = 'hogg-country-workspace';
const DB_VERSION = 1;
...
export function createDefaultProfile(): ManualProfile {
...
export async function importDocumentFiles(files: FileList | File[]): Promise<ImportedDocument[]> {
  ...
  for (const file of queue) {
    ...
    const record: ImportedDocument = {
      ...
      note: searchable
        ? ''
        : 'Stored locally. PDF text extraction is not wired up yet, so this file is searchable by title only for now.',
      importedAt,
      sizeBytes: file.size,
    };
...
export async function downloadManualExport(): Promise<void> {
  ...
  anchor.href = url;
  anchor.download = `${(profile.trailName || 'my').toLowerCase().replace(/[^a-z0-9]+/g, '-')}-field-manual.html`;
