...
function createReviewPacket(run, validation, importedRunPath, reviewPath, packetPath, scanPath, answerQualityScan, harnessContract) {
	...
	const lines = [
		...
		'',
		'```sh',
		`npm run review-status:scout-local-ai -- --run ${relative(REPO_ROOT, importedRunPath)} --review ${relative(REPO_ROOT, reviewPath)} --packet ${relative(REPO_ROOT, packetPath)} --scan ${relative(REPO_ROOT, scanPath)}`,
		'```',
		'',
		'Open the next unrated answer as a focused review card:',
		'',
		'```sh',
		`npm run review-status:scout-local-ai -- --run ${relative(REPO_ROOT, importedRunPath)} --review ${relative(REPO_ROOT, reviewPath)} --packet ${relative(REPO_ROOT, packetPath)} --scan ${relative(REPO_ROOT, scanPath)} --next`,
		'```',
		'',
		'The progress check may also print Human-reviewed batch helpers for standard unrated cases. Scan-flagged, safety, source-gap, missing-tool, provider-error, and confirmation-required cases are excluded from those helpers. Treat batch helpers as reading groups only: read every listed focused card first, then use the printed `rate-case --cases ...` command only when each answer honestly earned 5/5.',
		'',
